...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await query<any[]>('SELECT id FROM faculty WHERE email = ?', [email]);
    ...
    if (photoFile && photoFile.size > 0) photo_path = await saveUpload(photoFile, 'fac_photo');
    if (sigFile && sigFile.size > 0) signature_path = await saveUpload(sigFile, 'fac_sig');

    const tempToken = randomBytes(8).toString('hex');
    const uid            = await generateUID('faculty');
    const password_hash  = await bcrypt.hash(password, 12);

    const result = await query<any>(
      `INSERT INTO faculty
         (faculty_number, first_name, last_name, middle_name, subject, department,
          contact_number, address, photo_path, signature_path,
          qr_code_value, uid, email, password_hash, valid_until)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      [`FAC-TEMP-${tempToken}`, first_name, last_name, middle_name||null, subject||null,
       department, contact_number||null, address||null,
       photo_path||null, signature_path||null,
       `QR-TEMP-${tempToken}`, uid, email, password_hash,
       academicYear()]
    );
    const insertId = result.insertId;
    const faculty_number = `FAC-${String(insertId).padStart(4, '0')}`;
    const qr_code_value  = formatQRValue('FAC', String(insertId));
    await query('UPDATE faculty SET faculty_number=?, qr_code_value=? WHERE id=?', [faculty_number, qr_code_value, insertId]);

    return NextResponse.json({ success: true, faculty_number });
  } catch (e: any) {
    console.error(e);
